...
class Booking(models.Model):
    ...
    def __str__(self):
        return '{} for {} on {}'.format(self.name, self.package.name,
                                        self.start)


@receiver(post_save, sender=Booking)
def set_booking_permissions(sender, instance: Booking, **kwargs):
...
class PackagePermission(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['user', 'package'],
                                    name='unique_owner'),
        ]
    ...
    def __str__(self):
        if self.is_owner:
            ...
            fmt = '{} ({}) cannot write to {}'
        return fmt.format(self.user.username, self.user.id, self.package.name,
                          self.package.id)
